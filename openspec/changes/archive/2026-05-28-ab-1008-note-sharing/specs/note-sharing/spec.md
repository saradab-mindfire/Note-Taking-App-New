...
The system SHALL allow an authenticated note owner to generate a tokenized public share link for a note. The token MUST be a cryptographically random 64-character hex string. An optional `expiresAt` timestamp MAY be provided. Soft-deleted notes SHALL NOT be shareable.
...
- **THEN** the system creates a `shared_links` record with a unique token, `null` expiry, `null` revokedAt, and returns `201` with `{ token, shareUrl, expiresAt: null }`
...
- **THEN** the system creates the link record with the given `expiresAt` and returns `201` with the token and expiry echoed back
