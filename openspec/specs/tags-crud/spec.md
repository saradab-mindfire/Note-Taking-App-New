# tags-crud Specification

## Purpose
Provides CRUD operations for user-scoped tags, including creation, listing, updating, and deletion. Tags are owned by individual users, enforce a unique name constraint per user, and can be associated with notes via NoteTag join records. Shared Zod schemas in `packages/shared` serve as the single source of truth for validation on both backend and frontend.
