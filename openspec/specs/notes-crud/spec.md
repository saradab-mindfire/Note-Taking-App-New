...
The system SHALL allow an authenticated user to retrieve all their non-deleted notes. Soft-deleted notes (where `deletedAt IS NOT NULL`) SHALL be excluded. Each note in the list SHALL include its associated tags but SHALL NOT include the `content` field to keep response payloads small.

#### Scenario: List notes for user with notes
- **WHEN** an authenticated user sends `GET /notes` and has two active notes
- **THEN** the system SHALL return HTTP 200 with `{ success: true, data: [note1, note2] }` where each item omits `content`
...
#### Scenario: Soft-deleted notes excluded from list
- **WHEN** an authenticated user has one active note and one soft-deleted note
- **THEN** `GET /notes` SHALL return only the active note
